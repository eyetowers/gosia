...
func tokenize(payload string) (ParsedFrame, error) {
	...
	// Zero or more "[...]" data blocks (main data plus optional metadata
	// blocks emitted by the encoder). Per §5.4.2 the closing ']' terminates
	// the block; nesting is not permitted.
	for s.peek() == '[' {
		s.advance()
		if !s.skipUntil(']') {
			return ParsedFrame{}, fmt.Errorf("%w: unterminated data block", ErrMalformedFrame)
		}
	}
	...
	if s.peek() == '_' {
		...
		if !timestampRE.MatchString(ts) {
			return ParsedFrame{}, fmt.Errorf("%w: malformed timestamp %q", ErrMalformedFrame, ts)
		}
		s.pos = len(s.src)
	}

	if !s.eof() {
		return ParsedFrame{}, fmt.Errorf("%w: trailing data %q", ErrMalformedFrame, s.rest())
	}
